          ...
          >
            <div className="bg-canvas flex items-center justify-center max-h-[80vh] overflow-hidden">
              {isMurilo ? (
                ...
                <video src={item.mediaUrl} autoPlay controls loop playsInline className="w-full h-full object-contain max-h-[80vh]" />
              ) : (
                <img src={item.mediaUrl} alt={item.title} className="w-full h-full object-contain max-h-[80vh]" />
              )}
            </div>
            <div className="p-8 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
              ...
              <h2 className="font-display text-3xl text-ink leading-tight">{item.title}</h2>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-4 py-2 bg-amber text-canvas font-mono text-xs uppercase tracking-caps rounded hover:bg-amber-hot transition"
                >
                  {copied ? 'Copied' : 'Copy prompt'}
                </button>
                <a
                  href={item.sourceUrl} target="_blank" rel="noopener"
                  className="px-4 py-2 border border-rule text-ink-mute font-mono text-xs uppercase tracking-caps rounded hover:text-ink hover:border-ink-mute transition"
                >
                  Source
                </a>
                <button onClick={onClose} className="ml-auto text-ink-mute hover:text-ink text-2xl leading-none">×</button>
              </div>
